...
export class _Vector {
	...
	constructor(size: number = 0, refs: string = "", ...a: number[]) {
		...
		for (var i: number = 0; i < this.values.length; i++) {
			...
			if (!getterDict[this.refs[i]]) getterDict[this.refs[i]] = new Function(`return{get(){return this.get(this,"${this.refs[i]}");},set(value){return this.set(this,"${this.refs[i]}",value);}}`);
